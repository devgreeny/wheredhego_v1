...
def build_formation(players: list) -> dict:
    ...
    total_skill_players = len(buckets["WR"]) + len(buckets["TE"]) + 0
    ...
    # Verify we have enough skill players (aim for 6: QB, RB, 3 WR, TE)
    if len(formation_order) < 5:  # Minimum viable skill lineup
...
def generate_nfl_quiz_for_team_season(team: str, season: int, save_dir: str) -> tuple:
    ...
    for i, boxscore_url in enumerate(boxscore_urls):
        ...
        if i > 0:
            print("⏳ Waiting 5 seconds to be respectful to pro-football-reference...")
            time.sleep(5)
