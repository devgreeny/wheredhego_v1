...
# Use appropriate User model based on environment
...
def show_quiz():
    ...
    if request.method == "POST":
        ...
        has_played, existing_game_score = has_played_today('starting5')
        if has_played:
            # User already played today, redirect to show existing results
            flash("You've already played today's Starting5 quiz! Come back tomorrow for a new challenge.", "info")
            return redirect(url_for("starting5.show_quiz"))
        ...
        # Check if already played (backup check using session)
        ...
        # Save score to database - unified auth + backwards compatibility
        if not existing_score:
            # Save to unified MySQL system if logged in
            if current_user.is_authenticated:
                current_user.save_game_score(
                    ...
                    metadata={
                        'correct_answers': len([r for r in results if r == "✅"]),
            ...
            # Mark as played today for guests (logged-in users tracked via GameScore)
            mark_played_today('starting5')
        else:
    ...
    has_played_today_flag, _ = has_played_today('starting5')
